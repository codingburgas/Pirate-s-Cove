...
class RainTPL{
	...
	// -------------------------
		 ...
		 * @var string
		 */
		static $tpl_dir = "/home/maxcart/public_html/templates/";
		 ...
		 * @var string
		 */
		static $cache_dir = "/home/maxcart/public_html/cache/";
	 ...
	 * eg. 	$html = $tpl->display( 'demo', TRUE ); // return template in string
	 * or 	$tpl->display( $tpl_name ); // echo the template
	 *
	...
	function display( $tpl_name, $return_string = false ){
	 ...
	 *
	 * @param string $tpl_name Name of template (set the same of display)
	...
	protected function check_template( $tpl_name ){
		...
		if( !isset($this->tpl['checked']) ){
			...
			$this->tpl['tpl_filename']          = $tpl_dir . $tpl_basename;	// template filename  . '.' . self::$tpl_ext
			...
			elseif( preg_match( '/\{include="([^"]*)"(?: cache="([^"]*)"){0,1}\}/', $html, $code ) ){
				...
				if( isset($code[ 2 ]) ){
					...
					$compiled_code .= '<?php $tpl = new '.get_class($this).';' .
								 ...
								 '	$tpl->display( dirname("'.$include_var.'") . ( substr("'.$include_var.'",-1,1) != "/" ? "/" : "" ) . basename("'.$include_var.'") );'.
								 '} ?>';
				}
				else{
